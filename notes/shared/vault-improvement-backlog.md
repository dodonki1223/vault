...
Codex や Claude Code から Project 更新 workflow を呼びやすくする入口を検討する。

候補:

- `.agents/commands/project-update.md` を作る。
- Codex 側では必要に応じて `~/.codex/prompts/` に薄い wrapper を置く。
- Claude Code 側でも同じ workflow を参照する薄い wrapper にする。

workflow 本体は `.agents/workflows/project-update.md` を正とし、command 側に手順を重複させない。

## 次にやる候補

1. `classify-project-materials` skill を作る。
2. Project 更新 workflow を実際の Project で 1 回使い、重い部分を確認する。
3. `.agents/commands/project-update.md` を作り、slash command 的な入口を試す。
