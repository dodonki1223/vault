...
- Node.js の version は mise で固定することを検討する。
- `package.json` を作る場合は、build や application 化ではなく、vault 運用 script の入口として使う。
...
### Node.js による vault 運用 tool 管理

`ccusage` による token usage 確認や、skill / fetch skill の検証 script をまとめるため、Node.js を vault 運用 tool の実行基盤として使うことを検討する。

方針:

- Node.js の version は mise で固定する。
- `package.json` は vault 運用 command の入口として置く。
- 最初は TypeScript ではなく、Node.js 標準ライブラリ + `.mjs` で始める。
- TypeScript は、script が増えて型の恩恵が明確になってから検討する。
- build step や application 化はしない。
- usage log や集計結果は local-only にして commit しない。
...
{
  "scripts": {
    "usage:daily": "ccusage codex daily",
    "usage:session": "ccusage codex session",
    "skills:check": "node .agents/scripts/validate-skills.mjs"
  }
}
```
...
- `.mise.toml` で Node.js の major version だけ固定するか、patch version まで固定するか。
- `ccusage` を `devDependencies` に入れるか、`npx ccusage@latest` のまま使うか。
- `package-lock.json` を commit 対象にするか。
- README に mise / Node.js の setup 手順をどこまで書くか。
...
- Claude Code / Codex CLI の local usage を見る候補として `ccusage` を試す。
...
2. `ccusage` で足りる場合は、独自 script は作らず README か workflow に利用方法だけ残す。
