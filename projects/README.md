...
`status.md` は、Project の現在状況を同じ品質で読めるように作る。新規作成時の標準フォーマットは `create-project-note/templates/status.md` に置く。
...
- `status.md` を作成するときは `create-project-note/templates/status.md` の標準フォーマットを使う。
- `status.md` を更新するときは、最後に `概要`、`現在の状況`、`ブロッカー`、`未解決事項`、`次に見ること` の整合性を確認する。
...
新しい Project は `create-project-note` skill で作る。Project 作成時のインタビューと作成手順は skill 側に置き、作成ファイルのテンプレートは `create-project-note/templates/` に置く。

この README では、Project ディレクトリ全体の置き場、git 管理方針、更新ルールだけを管理する。

## 情報収集の並列化

既存 Project の状況を広く確認するときは、必要に応じて情報源ごとに read-only のサブエージェントを使う。サブエージェントは原則としてファイルを編集しない。各情報源から、事実、推測、重要リンク、未解決事項だけを短く返す。
