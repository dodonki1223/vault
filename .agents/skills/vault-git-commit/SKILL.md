...
```text
chore(vault): コミット手順を更新
chore(vault): note の保存場所ルールを更新
chore(vault): project ディレクトリの説明を更新
chore(skill): <skill-name> を更新
docs(notes): shared note の workflow を更新
docs(projects): project ディレクトリの README を更新
```
...
subject は簡潔かつ事実ベースにする。`chore(vault):`、`chore(skill):`、`docs(notes):`、`docs(projects):` などの prefix は英語のままでよいが、それ以外の説明は日本語で書く。何を変えたかを短く表し、diff を読めば分かる細部を詰め込まない。
