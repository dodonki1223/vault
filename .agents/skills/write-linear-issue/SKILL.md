...
実装タスク・調査タスクのどちらにも当てはまらない依頼（設定作業、確認依頼、ドキュメント整備など）はこちらを使う。
...
- `依頼内容` には、何をしてほしいか・何を確認してほしいかを箇条書きで書く。解決策を決めつけない。
- `関連` には Slack thread、Docs、PR、関連 issue への link だけを短く残す。本文に貼りすぎず、link と一言の理由だけにする。関連がなければ見出しを省略してよい。
