...
- この workflow は、単一対象（1つの issue / Project / milestone）に対する draft → 確認 → 書き込み → レビューという逐次パイプラインであり、並列化できる step はない。
- 書き込み（手順4）はレビュー（手順5）の前提であり、レビューは書き込み後の状態を確認するため、順序を入れ替えられない。
- 複数対象をまとめて依頼された場合も、書き込み前のユーザー確認を1つずつ挟むため、対象ごとに順番に処理する。並列化はしない。
- model の選択: 書き込みとレビューはいずれも人の確認を挟む判断が必要な処理のため、軽量 profile への委譲は行わない。
