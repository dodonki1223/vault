...
## Slack connector 設定・認証チェック

Slack tools が見つからない、Slack tool が認証エラーを返す、または Slack connector の状態が不明な場合は、取得を広げる前に次を行う。
...
2. 利用可能な Slack connector / tools を使う。Slack tools がまだ読み込まれておらず tool discovery が使える場合は、汎用 web search ではなく Slack tools を探す。tools がない、または認証状態が不明な場合は、先に `Slack connector 設定・認証チェック` を行う。
3. まず狭く取得する。
